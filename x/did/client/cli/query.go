...
func GetCmdStatesetDidFromMnemonic() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "get-stateset-did-from-mnemonic [mnemonic]",
		Short: "Get an stateset DID from a 12-word secret mnemonic",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
			...
			statesetDid, err := exported.FromMnemonic(mnemonic)
			if err != nil {
				return err
			}

			output, err := json.Marshal(statesetDid)
			if err != nil {
				panic(err)
			}
